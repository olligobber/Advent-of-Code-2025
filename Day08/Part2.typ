...
= Run UnionFind on Close Pairs until Two Groups Remain
...
#let representative(uf, i) = {
	if uf.at(1).at(i).at(0) == "root" {
		return (uf, i)
	}
	let (uf, root) = representative(uf, uf.at(1).at(i).at(1))
	uf.at(1).at(i).at(1) = root
	return (uf, root)
}
...
#let union(uf, i, j) = {
	...
	let ranki = uf.at(1).at(rooti).at(1)
	let rankj = uf.at(1).at(rootj).at(1)
	if ranki < rankj {
		uf.at(1).at(rooti) = ("child", rootj)
	} else if ranki > rankj {
		uf.at(1).at(rootj) = ("child", rooti)
	} else {
		uf.at(1).at(rooti) = ("child", rootj)
		uf.at(1).at(rootj) = ("root", rankj + 1)
	}
	return (uf.at(0) - 1, uf.at(1))
}
